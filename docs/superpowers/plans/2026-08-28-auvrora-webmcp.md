# Auvrora WebMCP Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-08-28-auvrora-webmcp-design.md`

## Global Constraints

- Product name in UI/docs/package metadata: `Auvrora WebMCP`; GitHub repository remains `AKzar1el/auvrora-webmcp`.
...
```bash
npm create cloudflare@latest -- /tmp/auvrora-c3 --framework=astro
```
...
git add package.json package-lock.json astro.config.mjs tsconfig.json wrangler.jsonc public src .gitignore LICENSE
 git commit -m "chore: scaffold Auvrora WebMCP worker app"
```
...
Every `observedEvidence` string must be generated by Auvrora, not copied wholesale. Examples:
...
`createAuvroraStore()` returns `getState`, `subscribe`, `replaceAudit`, `setScope`, `setVerification`, and `reset`.
...
**Interfaces:**
- Produces one `AuvroraController` object used later by WebMCP:
...
git add src/lib/app src/components src/pages/index.astro src/styles/global.css test/controller.test.ts
 git commit -m "feat: build shared Auvrora human workflow"
```
...
- DigestSEO informed the workflow and some audit-hardening concepts, but private commercial source is not required to run Auvrora;
...
git add .github README.md docs
 git commit -m "docs: prepare Auvrora WebMCP challenge release"
```
...
The plan uses one `AuditRun`, `Finding`, `AuvroraState`, `VerificationResult`, and `AuvroraController` contract throughout. Tool handlers consume only the controller; UI events consume only the controller; fetch/analyzer modules remain server-side pure services.
