...
> **Live demo:** https://auvrora-webmcp.tomi-seregi99.workers.dev

## The loop

`Audit → Inspect → Scope → Implement outside Auvrora → Re-audit → Verify`

Auvrora does not modify the target website, run an AI backend, accept credentials, or expose fetched HTML to the agent. Its initial audit is deliberately small and deterministic.
...
```bash
git clone https://github.com/AKzar1el/auvrora-webmcp.git
cd auvrora-webmcp
npm ci
npm run dev
```
...
1. **ChatGPT desktop app:** open Auvrora in ChatGPT's built-in browser. When site tools are available for the account, ChatGPT discovers the tools provided by the page.
2. **Google Chrome 149+:** open `chrome://flags/#enable-webmcp-testing`, enable WebMCP testing, relaunch Chrome, then open Auvrora.
...
The numeric title/description ranges are **Auvrora heuristics, not Google ranking requirements**. Google documents that title links and snippets may be truncated or generated differently depending on context and device width.
